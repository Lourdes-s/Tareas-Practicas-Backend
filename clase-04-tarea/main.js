...
class ManejadorEmpleados {
    ...
    agregarEmpleado(nombre, sueldo, fecha_de_contratacion, tipo) {
        if (!this.tipos_permitidos.includes(tipo)) {
            console.error('Error: El tipo de empleado no es valido.');
        }
        else {
            const nuevo_empleado = new Empleado(nombre, sueldo, fecha_de_contratacion, this.contador_id++, tipo);
...
const manejador_empleados = new ManejadorEmpleados(1);
